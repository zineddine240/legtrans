...
if (!fs.existsSync(screenshotPath)) {
  ...
  // Check if we can find any other png file in the brain folder
  process.exit(1);
}

console.log("Found screenshot. Size:", fs.statSync(screenshotPath).size, "bytes");

async function testWithLangs(useLangs) {
  console.log(`\n--- Testing with useLangs = ${useLangs} ---`);
...
async function run() {
  // Test without langs first (to see if it reproduces empty result)
  await testWithLangs(false);
  // Test with langs (to see if it extracts Arabic text)
  await testWithLangs(true);
}
